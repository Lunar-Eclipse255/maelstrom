...
namespace maelstrom {
    ...
    namespace logging {
        ...
        std::string rtf_file_header = "{\\rtf1\\ansi\\ansicpg1252\\cocoartf2761 \\cocoatextscaling0\\cocoaplatform0{\\fonttbl\\f0\\fswiss\\fcharset0 Helvetica;}\n{\\colortbl;\\red255\\green0\\blue0;\\red0\\green255\\blue0;\\red47\\green32\\blue66;}\n{\\*\\expandedcolortbl;;\\csgenericrgb\\c100000\\c0\\c0;}\n\\margl1440\\margr1440\\vieww11520\\viewh8400\\viewkind0\n\\pard\\tx720\\tx1440\\tx2160\\tx2880\\tx3600\\tx4320\\tx5040\\tx5760\\tx6480\\tx7200\\tx7920\\tx8640\\pardirnatural\\partightenfactor0\n\n";
        ...
        bool* init(bool run_error_log, bool run_data_log, std::vector<int> left_motor_ports, std::vector<int> right_motor_ports, int battery_level){
            ...
            if (run_error_log) {
                ...
                //std::string error_log_filename = log_folder_path + std::string("/error_logfile_") + get_current_date_time(E_DATE_TIME) + ".rtf";
                std::fstream error_log_file;
                error_log_file.open(error_log_filename, std::ios::out);
                if (error_log_file.is_open()) {
                    printf("%s created successfully\n", error_log_filename.c_str());
                    error_log_file << rtf_file_header << "\\cf2 Program Started: \\line \\line }";
            ...
            if(run_data_log) {
                ...
                //std::string data_log_filename = log_folder_path + std::string("/data_logfile_") + get_current_date_time(E_DATE_TIME) + ".rtf";
                std::fstream data_log_file;
                data_log_file.open(data_log_filename, std::ios::out);
                if (data_log_file.is_open()) {
                    printf("%s created successfully\n", data_log_filename.c_str());
                    data_log_file << rtf_file_header << "\\cf2 Program Started: \\line \\line }";
                    data_log_file.close();
                }
                else{
                    init_arr[1] = false;
                    ...
                    if (motor_connected(motor_ports.at(port_index))){
                        if (motor_fault & pros_motor_faults[i].first) {
                            if (!faults[port_index][i]){
                                error_log_file.seekp((long) error_log_file.tellp() - 1) << "Motor: " + std::to_string(abs(motor_ports.at(port_index))) + pros_motor_faults[i].second + get_current_date_time() + "\\line }";
                                faults[port_index][i] = true;
                            }
                        }
                        else {
                            if (faults[port_index][i]){
                                error_log_file.seekp((long) error_log_file.tellp() - 1) << "Motor: " + std::to_string(abs(motor_ports.at(port_index))) + pros_motor_faults[i].second + "all clear: " + get_current_date_time() + "\\line }";
                            }
                ...
                while (true) {
                    ...
                    if (!pros::competition::is_disabled()){
                        if (pros::competition::is_autonomous() && !auton_start) {
                            error_log_file.seekp((long) error_log_file.tellp() - 1) << "Auton: " + get_current_date_time() + "\\line \\line }";
                            auton_start = true;
                        }
                        else if (!pros::competition::is_autonomous() && !driver_start){
                            error_log_file.seekp((long) error_log_file.tellp() - 1) << "\\line " + std::string("Driver: ") + get_current_date_time() + "\\line \\line }";
                            driver_start = true;
                        }
                        for (int i = 0; i < motor_ports.size(); i++){
                            error_log_file.close();
                            ...
                            if (!(motor_connected(motor_ports.at(i)))) {
                                if (!faults[i][4]){
                                    error_log_file.seekp((long) error_log_file.tellp() - 1) << "Motor: " + std::to_string(abs(motor_ports.at(i))) + " disconnected: " + get_current_date_time() + "\\line }";
                                    faults[i][4] = true;
                                }
                            }
                            else{
                                if (faults[i][4]){
                                    error_log_file.seekp((long) error_log_file.tellp() - 1) << "Motor: " + std::to_string(abs(motor_ports.at(i))) + " reconnected: " + get_current_date_time() + "\\line }";
                                }
                                faults[i][4] = false;
                            }
                        }
                        if (!(battery(battery_threshold))) {
                            if(!battery_below_threshold){
                                error_log_file.seekp((long) error_log_file.tellp() - 1) << "Battery below " + std::to_string(battery_threshold) + "% " + get_current_date_time() + "\\line }";
            ...
            if (init_arr[1]) {
                while (true) {
                    ...
                    if (!robot_coords_vector.empty()) {
                        ...
                        data_log_file.seekp((long) data_log_file.tellp() - 1) << "x: " + format_coords(robot_coords_vector.at(0)) + " y: " + format_coords(robot_coords_vector.at(1)) + " theta: " + format_coords(robot_coords_vector.at(2)) + " " + get_current_date_time() + "\\line }";
                        data_log_file.close();
                    }
                    coords_mutex.give();
                    pros::delay(500);
        ...
        void write_to_file(std::string message, log_file file) {
            if (file == E_ERROR_LOG){
                if (init_arr[0]) {
                    std::fstream error_log_file;
                    error_log_file.open(error_log_filename, std::ios::app);
                    error_log_file.seekp((long) error_log_file.tellp() - 1) << message + " " + get_current_date_time() + "\\line }";
                    error_log_file.close();
                }
            }
            else{
                if (init_arr[1]) {
                    std::fstream data_log_file;
                    data_log_file.open(data_log_filename, std::ios::app);
                    data_log_file.seekp((long) data_log_file.tellp() - 1) << message + " " + get_current_date_time() + "\\line }";
                    data_log_file.close();
                }
            }
        }
        ...
        void task_complete(std::string task_name, bool completion) {
            if (init_arr[0]) {
                ...
                error_log_file.open(error_log_filename, std::ios::app);
                if (completion){
                    error_log_file.seekp((long) error_log_file.tellp() - 1) << task_name + " Complete " + get_current_date_time() + "\\line }";
                }
                else{
                    error_log_file.seekp((long) error_log_file.tellp() - 1) << task_name + " Incomplete " + get_current_date_time() + "\\line }";
                }
            }
        }
    }
